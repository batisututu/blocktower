extends SceneTree
## 벽돌 외벽 원본: 줄눈이 분리된 벽돌, 석재 띠장, 사각 격자창.
...
func _render() -> void:
	...
	for part in ["floor_entry","floor_wide","floor_mid","floor_top"]:
		...
		assert(ResourceSaver.save(packed,scene_directory.path_join("brick_"+part+".tscn")) == OK)
		await process_frame
		await process_frame
		await RenderingServer.frame_post_draw
		var image := viewport.get_texture().get_image()
		assert(image.save_png(OUTPUT.path_join("brick_"+part+".png")) == OK)
	...
	var file := FileAccess.open(OUTPUT.path_join("brick_geometry.json"),FileAccess.WRITE)
	file.store_string(JSON.stringify(records,"\t"))
	file.close()
	print("BRICK_TOWER_RENDER ",records)
	viewport.queue_free()
	await process_frame
	quit()
