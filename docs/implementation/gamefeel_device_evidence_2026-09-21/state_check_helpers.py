...
E=ROOT/'docs/implementation/gamefeel_device_evidence_2026-09-21'
...
def state(name):
 states=[]
 for n in ['a','b']:
  raw=adb('shell','run-as','com.blocktower.game','cat','files/save_v1/slot_'+n+'.json');(E/(name+'_slot_'+n+'.json')).write_bytes(raw)
