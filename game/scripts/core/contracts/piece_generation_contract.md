...
- The personal app starts new saves with `piece_generator_low_single.tres` (single weight 1 of total 41). Existing saves retain `piece_generator_default.tres` (single weight 5 of total 45) through their checkpoint hash. A save is never silently replayed with the other profile. Phase 4 server challenges store an immutable `supply_profile`; existing challenges use `classic`, and weeks beginning 2026-09-28 UTC use `reduced_single`.
