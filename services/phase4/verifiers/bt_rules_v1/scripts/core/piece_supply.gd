extends RefCounted
## Blocktower 자체 조사 예제를 이식한 공급 코드. 외부 코드를 벤더링한 파일이 아니다.
## 고정 실험 데이터이며 제품 밸런스는 미확정이다. 4.7.2 실행 증거는 W0/W1 보고서를 따른다.
# 출처: docs/examples/piece_supply_v0_1.gd 를 2026-09-21 W1에서 복사했다. 원본은 보존한다.
# FAMILIES 정의와 함수 본문은 원본과 동일하다. definition_hash()는 FAMILIES 데이터만
# 직렬화하므로 이 주석 추가는 해시(a91e1b38...)에 영향을 주지 않는다.
# W2에서 계약(contracts)으로 옮길 때 추첨·판정 계약은 유지하고 정의만 읽기 전용으로 분리한다.
